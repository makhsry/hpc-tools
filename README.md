...
This repository contains a specialized toolset for preparing and submitting high-performance computing (HPC) jobs.

## `prepare_and_submit_job_to_sockeye_or_computecanada.sh`

This script is a comprehensive Bash-based utility designed to automate the configuration and submission of Slurm-managed jobs. It handles cluster-specific hardware architectures, resource allocation logic, and job recovery. It is specifically tailored for **COMSOL Multiphysics** simulations. The script primaryly manages the entire workflow from environment validation to job submission on various Canadian HPC clusters.

### 1. Logic and Resource Allocation

The script implements a deterministic allocation of resources based on the selected cluster's hardware profile.

#### Hardware Resource Mapping
The script maps user-selected options to specific Slurm assignments for $N_{cpu}$ (cores), $M_{total}$ (RAM), and $Arch$ (microarchitecture). 

### 2. Comprehensive Cluster Hardware Profiles

#### **Sockeye (UBC ARC)**
| Option | RAM/CPU | $M_{total}$ | $N_{cpu}$ | Arch | isGPU | Note |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
### 3. Full Execution Workflow Architecture
...
### 4. Interactive Usage Walkthrough (Step-by-Step)
...
### 5. Detailed Command Line Construction Logic
...
### 6. Technical Summary of Environment Controls
